...
def summary_to_rows(summary: BenchmarkSummary) -> list[dict[str, object]]:
    ...
    for metrics in summary.backend_metrics:
        quality = quality_lookup[(metrics.backend_name, metrics.dataset_name)]
        cost = cost_lookup[(metrics.backend_name, metrics.dataset_name)]
        rows.append(
            {
                ...
                "quality": round(quality.aggregate_quality, 4),
                "cost_per_million_tokens_usd": round(cost.cost_per_million_tokens_usd, 2),
            }
        )
    return rows
...
def render_markdown(summary: BenchmarkSummary) -> str:
    ...
    for row in rows:
        row_text = (
            ...
            f"{row['quality']} | {row['cost_per_million_tokens_usd']} |"
        )
...
def render_html(summary: BenchmarkSummary) -> str:
    ...
    bars = "".join(
        (
            ...
            f"<td>{row['latency_p95_ms']}</td><td>{row['quality']}</td></tr>"
        )
        for row in rows
    )
